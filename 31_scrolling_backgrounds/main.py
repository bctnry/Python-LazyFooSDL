...
class Dot:
    ...
    def move(self):
        ...
        if self.pos_x < 0 or (self.pos_x + DOT_WIDTH > SCREEN_WIDTH):
            self.pos_x -= self.vel_x

        self.pos_y += self.vel_y
        if self.pos_y < 0 or (self.pos_y + DOT_HEIGHT > SCREEN_HEIGHT):
            self.pos_y -= self.vel_y

    def render(self, cam_x: int = 0, cam_y: int = 0):
    ...
    if not init():
        ...
        if not load_media():
            ...
            while not quit:
                ...
                dot.move()
                scrolling_offset -= 1
                if scrolling_offset < -g_bg.get_width(): scrolling_offset = 0

                sdl2.SDL_SetRenderDrawColor(g_renderer, 0xff, 0xff, 0xff, 0xff)
                sdl2.SDL_RenderClear(g_renderer)
                
                g_bg.render(scrolling_offset, 0)
                g_bg.render(scrolling_offset+g_bg.get_width(), 0)
                dot.render()
