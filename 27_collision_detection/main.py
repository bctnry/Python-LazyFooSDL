...
class Dot:
    ...
    def move(self, wall: sdl2.SDL_Rect = None):
        self.pos_x += self.vel_x
        self._collider.x = self.pos_x
        # NOTE: we have to update self._collider here for the following collision check.
        # or else we will have the following events:
        #     pos_x += vel_x  (collide, but self._collider wasn't updated.)
        #     check_collision(self._collider, wall)  (reports "no collision" so
        #                                             the dot does not go back)
        # if the wall is not thin enough the dot will stuck at the border.
        if self.pos_x < 0 or (self.pos_x + DOT_WIDTH > SCREEN_WIDTH) or check_collision(self._collider, wall):
            self.pos_x -= self.vel_x
            self._collider.x = self.pos_x
        ...
        if self.pos_y < 0 or (self.pos_y + DOT_HEIGHT > SCREEN_HEIGHT) or check_collision(self._collider, wall):
            self.pos_y -= self.vel_y
            self._collider.y = self.pos_y
...
def check_collision(a: sdl2.SDL_Rect, b: sdl2.SDL_Rect):
    left_a = a.x
    right_a = a.x + a.w
    top_a = a.y
    bottom_a = a.y + a.h
